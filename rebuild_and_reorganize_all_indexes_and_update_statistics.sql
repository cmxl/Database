...
FETCH NEXT
FROM FragmentedTableList INTO @tableName,
    @indexName,
    @indexId,
    @indexType,
    @percentFragment,
    @spaceUsed,
    @pageCount WHILE @@FETCH_STATUS = 0 BEGIN PRINT 'Processing [' + @indexName + '] on table [' + @tableName + '] which is ' + cast(@percentFragment AS NVARCHAR(50)) + '% fragmented' IF (@percentFragment <= @maxFragmentation) BEGIN EXEC (
